...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        if self.current_state.id == "heat":
            ...
            if temp > self.setPoint:
                redLight.off()
                blueLight.pusle(fade_in_time = 0.8, fade_out_time = 0.8, n = None, background = True)
            else:
                blueLight.on()
                redLight.off()
        else:
    ...
    def setupSerialOutput(self):
        ...
        output = f"{state}, {tempF:.0f},{self.setPoint:.0f}"
    ...
    def _lcdLine1(self) -> str:
        return datetime.now().strftime("%b %d %H:%M:%S").ljust(16) + "\n"

    def _lcdLine2_temp(self) -> str:
        from math import floor
        return f"Temp: {floor(self.getFahrenheit()):.0f} F".ljust(16)

    def _lcdLine2_state(self) -> str:
        return f"{self.current_state.id.upper()} SP:{self.setPoint:.0f}F".ljust(16)

    def _instantLCDRefresh(self):
        screen.updateScreen(self._lcdLine1() + self._lcdLine2_state())
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            lcd_line_1 = datetime.now().strftime("%m/%d %H:%M:%S").ljust(16) + "\n"
    
            ## Setup Display Line 2
            if altCounter < 6:
                lcd_line_2 = self._lcdLine2_temp()
                altCounter += 1
            
            else:
                lcd_line_2 = self._lcdLine2_state()
                altCounter += 1
                if altCounter >= 11:
                    self.updateLights()
                    altCounter = 1
